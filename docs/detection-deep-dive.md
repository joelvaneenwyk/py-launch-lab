...
Consider `lab-gui.exe` in a uv venv:

```
lab-gui.exe        PE: GUI (wrapper generated by pip/uv)
  └── pythonw.exe  PE: CUI (uv trampoline — should be GUI!)
        └── conhost.exe
```

Direct detection of `lab-gui.exe`:
- `detect_console_host(lab_gui_pid)` → **False** (conhost is grandchild)
- `detect_visible_window(lab_gui_pid)` → **True** (may have visible window)
